...
int32_t cbb_ate_info_text1(ffw_handle_t e, const void *text, int32_t len);
...
int32_t cbb_ate_info_data1(ffw_handle_t e, const void *data, int32_t len, bool flag);
...
/**
 * @brief AT命令处理完毕，任意格式的错误信息，如果 @c data 为 NULL 或者 @c len 为0，在什么都不发送，只结束当前AT执行
 * 
 * @param e  AT引擎
 * @param data 
 * @param len 
 * @return int32_t 
 */
int32_t cbb_ate_result_data(ffw_handle_t e, const void *data, int32_t len);
...
void cbb_ate_resp_finish(ffw_handle_t e);
...
bool cbb_ate_be_empty_str_with_quote(void *p, int32_t idx);
#if (defined(CONFIG_FFW_UNI_MQTT_ENABLE) || defined(CONFIG_FFW_MQTT_CLOUD_ENABLE) || defined(CONFIG_FFW_EC200_MQTT_AT_ENABLE))
bool cbb_ate_be_empty_str_with_no_quote(void *p, int32_t idx);
#endif
 ...
 * @return uint32_t 波特率
 */
uint32_t cbb_ate_get_baud(ffw_handle_t e);
