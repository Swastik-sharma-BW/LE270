...
void ffw_thread_sleep_us(uint32_t usec);
...
/**
* @brief 删除Threadid task
* 
 */
void ffw_thread_delete1(uint32_t Thread_id);
 ...
 * 
 * @param sem 信号量句柄 
 * @param count_ptr 保存信号量计数的指针
 * @return int 
 * @retval FFW_R_SUCCESS: 成功
 * @retval FFW_R_FAILED: 失败
 */
int ffw_sem_get_count(ffw_handlep_t sem, uint32_t *count_ptr);
...
* @retval FFW_R_FAILED: 失败
 */
int ffw_set_timestamp(const struct ffw_timeval *t);
...
/**
 * @brief 在AT线程执行某个函数，如果队列满了，阻塞直至发送成功
 * 
 * @param cb 函数
 * @param arg 数据
 * @param sync 是否同步
...
/**
 * @brief 在AT线程执行某个函数，如果队列满了，直接返回错误
 * 
 * @param cb 函数
 * @param arg 数据
 * @param sync 是否同步
 * @return int 
 */
int ffw_at_thread_callback_timeout(void (*cb)(void *arg), void *arg, bool sync, uint32_t timeout);
...
/**
 * @brief 分配内存，这个函数不能用malloc(nmemb * size)代替，否则mbedtls有时候有问题，不知道原因
 * 
 * @param nmemb 块的个数
 * @param size 块的大小
 * @return void* 内存首地址
...
void *ffw_psram_malloc_plat(uint32_t size);
void *ffw_psram_calloc_plat(uint32_t nmemb, uint32_t size);
void ffw_psram_free_plat(void *p);
int  ffw_get_psram_memory_info1(uint32_t *total, uint32_t *ava_size, uint32_t *max_block_size);
/**
 * @brief 
 * 
 * @param dest 
...
* @return int
 */
int ffw_get_bootcase(int *bootcase);
...
bool ffw_soft_reset_reason(int resaon);
...
* @return uint32_t 
 */
uint32_t ffw_disable_interrupts();
...
* @return uint32_t 
 */
void ffw_restore_interrupts(uint32_t mask);
...
int ffw_enable_wdt(bool en);

void ffw_feed_dog(void);

bool ffw_at_send(const uint8_t *pCmdStr, uint32_t len);

int32_t ffw_pal_secboot_enable(void);
int32_t ffw_pal_secboot_get_enable(uint32_t *enable);
...
* @param ... 
 */
void ffw_log(const char *fmt, ...);
void ffw_less_log(const char *fmt, ...);


void ffw_no_log(const char *fmt, ...);
