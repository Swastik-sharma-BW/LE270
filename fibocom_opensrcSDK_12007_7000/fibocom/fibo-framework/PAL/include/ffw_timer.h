...
* @retval FFW_R_FAILED: 失败
 */
int ffw_timer_start(ffw_timerp_t timer, uint32_t timeout, bool periodic);
...
* @retval FFW_R_FAILED: 失败
 */
int ffw_timer_microsecond_start(ffw_timerp_t timer, uint32_t timeout, bool periodic);
...
uint32_t ffw_timer_remaining(ffw_timerp_t timer);
#endif
#if defined(CONFIG_CUST_XIAONIU_FEATURE) || defined(CONFIG_CUST_NBSANXING_FEATURE)
uint32_t ffw_timer_new(uint32_t ms, void (*fn)(void *arg), void *arg);
int ffw_timer_free(uint32_t timerid);
uint32_t ffw_timer_period_new(uint32_t ms, void (*fn)(void *arg), void *arg);
#endif
