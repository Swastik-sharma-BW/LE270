...
* @return int 
 */
int ffw_sign_init(ffw_md_type_t md_type, bool hmac, const uint8_t *key, int key_len, ffw_handle_t *ctx);
...
* @return int 
 */
int ffw_sign_update(ffw_handle_t ctx, const uint8_t *payload, int payload_len);
...
* @return int 
 */
int ffw_sign_finish(ffw_handle_t ctx, ffw_encode_type_t fmt, ffw_buffer_t *out);
