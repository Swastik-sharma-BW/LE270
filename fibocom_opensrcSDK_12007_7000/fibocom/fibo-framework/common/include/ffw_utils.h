...
#define FFW_OTA_LOG_FILE FFW_BASE_DIRNAME FFW_FS_SEP "ffw_ota_log.bin"
...
#define ffw_get_memory(size)                                            \
    ({                                                                     \
        ...
        {                                                                  \
            ffw_add_memInfo(p, size, __LINE__, __FILE__, __FUNCTION__); \
...
#define ffw_get_memory_mock(p)                                       \
    ({                                                                  \
        ...
        if (ffw_should_print_memory_use_info())                      \
        {                                                               \
            ffw_add_memInfo(p, 0, __LINE__, __FILE__, __FUNCTION__); \
...
#define ffw_strdup(str)                                 \
    ({                                                     \
        ...
        if ((str) != NULL)                                 \
        {                                                  \
            int __size = (int)strlen(str);                 \
            p = ffw_get_memory((size_t)__size + 1);     \
            if (p != NULL)                                 \
            {                                              \
                ffw_memcpy(p, (str), (uint32_t)__size); \
            }                                              \
        }                                                  \
        p;                                                 \
    })
...
* @return int 
 */
int ffw_add_memInfo(void *ptr, uint32_t bytes, int line, const char *filename, const char *functionName);
...
* @return int 
 */
int ffw_dump_memInfo(void (*cb)(void *ptr, uint32_t bytes, int line, const char *filename, const char *functionName, uint32_t max_alloc, uint32_t total, void *arg), void *arg);
...
* @return int 
 */
int ffw_delete_memInfo(void *ptr);
