...
* @return int 
 */
int ffw_rbuffer_resize(ffw_rbuffer_t *rbuf, uint32_t size);
...
/**
* @brief 只给 ffw_msg_queue_try_put_front 使用，其他不能使用
* 
* @param rbuf 
* @param wr_data 
* @param size 
...
* @return int 
 */
int ffw_rbuffer_write_all_front(ffw_rbuffer_t *rbuf, const void *wr_data, int size, int timeout);
...
/**
* @brief 停止唤醒buffer，此时read函数会返回 FFW_R_FAILED
* 
* @param rbuf 
* @return int 
 */
int ffw_rbuffer_stop(ffw_rbuffer_t *rbuf, bool drain);
