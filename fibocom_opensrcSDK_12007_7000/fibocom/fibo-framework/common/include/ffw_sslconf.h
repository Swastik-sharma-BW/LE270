...
//  PSK密钥
#define FFW_PSKKEY_FILE (3)

//  PSK实体
#define FFW_PSKID_FILE (4)

/// 附加公钥类型证书
#define FFW_CERT_EXT_FILE (5)

/// 附加私钥类型证书
#define FFW_KEY_EXT_FILE (6)

//2025-01-15, mahongmei, MTC1007-35,Begin. fix qidnscfg bug when ssl marco is closed.
#ifndef FFW_SSL_SOCKET_ID_MAX
#define FFW_SSL_SOCKET_ID_MAX (16)
#endif
//2025-01-15, mahongmei, MTC1007-35,End. 
...
///除了SSL3.0以外的版本
#define FFW_SSL_VER_NO_SSL3_0 (7)

///TLS1.3
#define FFW_SSL_VER_TLS1_3 (6)
...
* 
* @param id 配置ID
* @param type 证书类型 ::FFW_CERT_FILE 公钥 ::FFW_KEY_FILE 私钥 ::FFW_TRUST_FILE CA证书 ::FFW_PSKKEY_FILE PSK密钥 ::FFW_PSKID_FILE PSK实体
* @param data 证书或PSK数据
...
* @retval FFW_R_FAILED: 失败
 */
int ffw_sslconf_get_file(uint32_t id, const ffw_cert_t **cert_file, const ffw_cert_t **key_file, const ffw_cert_t **trustfile, int *trustfile_num, int* pskkey, int* pskid);
...
/**
* @brief 获取TLS支持的算法信息，包括算法ID和算法名字
* 
* @param cb 回调函数
* @param arg 回调函数上下文
* @return int 算法的个数
 */
int ffw_ssl_support_cipherlist(void (*cb)(int idx, int cipherid, const char *ciphername, void *arg), void *arg);
...
* @return int 
 */
int ffw_sslconf_get_cipher_count();
...
int ffw_sslconf_use_socket_id();

int ffw_sslcert_dump_to_file(const char *filename);

int ffw_sslcert_load_from_file(const char *filename);
