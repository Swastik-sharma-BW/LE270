...
* @param family FFW_AF_INET: ipv4地址 FFW_AF_INET6: ipv6地址 
...
* @retval FFW_R_FAILED: 失败 
 */
int ffw_localaddr(ffw_handle_t iface, int family, uint16_t port, struct ffw_sockaddr *laddr, int *laddrlen);
...
/**
 * @brief reattach for ecm data call
 *
 * @return int
 * @retval FFW_R_SUCCESS: 成功
 * @retval FFW_R_FAILED: 失败
*/
int ffw_ecm_reattach(uint8_t simid);

uint8_t ffw_get_pdp_type(uint8_t cid);
