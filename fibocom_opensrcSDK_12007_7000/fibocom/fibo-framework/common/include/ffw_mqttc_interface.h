...
/// destory
#define ffw_mqttc_i_destroy(c) ((c)->destroy)(c)
...
struct ffw_mqttc_interface_s
{
    ...
    /// 订阅
    int (*sub)(ffw_mqttc_interface_t *c, const char *topic, int topic_len, int qos);

    /// 去订阅
    int (*unsub)(ffw_mqttc_interface_t *c, const char *topic, int topic_len);

    /// 发布
    int (*pub)(ffw_mqttc_interface_t *c, const char *topic, int topic_len, int qos, bool retain, bool dup, const uint8_t *payload, uint16_t payload_len);

    /// 更新遗嘱topic
    int (*willtopicud)(ffw_mqttc_interface_t *c, const char *topic, int qos, bool retain);

    /// 更新遗嘱消息
    int (*willmsgud)(ffw_mqttc_interface_t *c, const char *message);

    /// 注册主题
    int (*topic_reg)(ffw_mqttc_interface_t *c, const char *topic);
