...
typedef enum {
     ...
     */
    BOT_MSG_INIT_SUCCESS,
    /**
     * @brief device network reconnection to platform
     * 1. for gateway devices, no processing is required and they will automatically reconnect internally as a notification only
     * 2. for subdevice, after receiving this notification, it is necessary to login to all sub devices again use bot_gw_subdev_login()
     */
    BOT_MSG_RECONNECT_SUCCESS,
    /**
