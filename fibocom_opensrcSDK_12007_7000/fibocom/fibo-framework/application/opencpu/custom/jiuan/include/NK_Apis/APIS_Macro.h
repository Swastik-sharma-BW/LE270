...
#define NK_SWAP64(__v) (((NK_UInt64)(NK_SWAP32((__v >> 32) & 0xffffffff)) << 32) | (NK_SWAP32(__v & 0xffffffff)))
...
NK_STR_TRIM(NK_PChar text, NK_PChar stack, NK_Size stack_len)
{
	...
	for (start_pos = text; start_pos < text + text_len + 1; ++start_pos) {
		if (' ' != start_pos[0]) {
			for (i = 0; i < text_len && i < stack_len; ++i) {
				stack[i] = start_pos[i];
			}
			stack[i] = '\0';
			break;
 ...
 */
static inline NK_Boolean NK_IS_LITTLE_END(void)
{
	union{
    	   NK_UInt32 dword;
    	   NK_UInt8 bytes[4];
...
#ifndef list_offsetof
#define list_offsetof(type, member) ((long) &((type *) 0)->member)
#endif

#define dl_list_entry(item, type, member) \
        ((type *) ((char *) item - list_offsetof(type, member)))
