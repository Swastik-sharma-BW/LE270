...
/// socket创建之后的回调函数，可在回调函数里设置sockopt
typedef int (*ffw_customer_dns_get_callback_t)(int family, const char **primary_server, const char **second_server, void *arg);
...
#define ffw_transport_connnect(t, lport, host, port)                                              \
    ({                                                                                               \
        ...
        if ((t)->init && (t)->connect)                                                               \
        {                                                                                            \
            ret = (((t)->connect)((t), (lport), (host), (port), ((t)->connect_timeout), FFW_AF_UNSPEC)); \
        }                                                                                            \
        ret;                                                                                         \
    })
...
/// get sockname
#define ffw_transport_set_sockfd_callback(t, cb, dnscallback, arg) \
    ({                                                                \
        int ret = FFW_R_FAILED;                                    \
        if ((t)->init && (t)->set_sockfd_callback)                    \
        {                                                             \
            ret = (t)->set_sockfd_callback(t, cb, dnscallback, arg);  \
        }                                                             \
        ret;                                                          \
    })
...
* @param family FFW_AF_INET 表示连接IPV4地址 FFW_AF_INET6 表示连接IPV6地址，FFW_AF_UNSPEC 表示任意一个
* @return int
...
/**
* @brief UDP发送数据函数指针，需要指定远端地址，地址类型为 @c struct ffw_sockaddr *
* 
* @param h transport 句柄
* @param buf 数据缓冲区
* @param len @c buf 的大小
...
* @retval FFW_R_FAILED 失败
 */
typedef int (*ffw_transport_sendto1_t)(ffw_transport_t *h, const void *buf, uint32_t len, struct ffw_sockaddr *dest, int destlen);
...
* @retval FFW_R_FAILED 失败
 */
typedef int (*ffw_transport_recvfrom1_t)(ffw_transport_t *h, void *buf, uint32_t len, uint32_t *read_len, struct ffw_sockaddr *from, int *fromlen);
...
* @retval FFW_R_FAILED 失败
 */
typedef int (*ffw_transport_get_dstaddr_t)(ffw_transport_t *t, struct ffw_sockaddr *addr, int *addrlen);
...
* @param dns_callback 后去dns服务器的回调函数
* @return int
* @retval FFW_R_SUCCESS 成功
* @retval FFW_R_FAILED 失败
 */
typedef int (*ffw_transport_set_sockfd_callback_t)(ffw_transport_t *t, ffw_socket_callback_t callback, ffw_customer_dns_get_callback_t dns_callback, void *arg);
...
int ffw_transport_fact_init1(ffw_transport_t *t, ffw_transport_type_t type, uint32_t id);
