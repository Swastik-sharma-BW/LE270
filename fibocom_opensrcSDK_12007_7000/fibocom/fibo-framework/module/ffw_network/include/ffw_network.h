...
struct ffw_net_handler_s
{
    ...
    ///FFW_AF_INET for ipv4, FFW_AF_INET6 for ipv6
    int family;
    ...
    struct ffw_sockaddr_storage dst_addr;
    ...
    /// 连接协议 FFW_IPPROTO_TCP 或者 FFW_IPPROTO_UDP
    int proto;
    ...
    /// 获取调用者自定义的dns服务器地址
    ffw_customer_dns_get_callback_t dns_get_cb;
...
* @retval FFW_R_FAILED: 失败
 */
int ffw_net_get_dstaddr(ffw_net_handler_t *client, struct ffw_sockaddr *addr, int *addrlen);
 ...
 * @retval FFW_R_FAILED: 失败 
 */
int ffw_net_sendto1(ffw_net_handler_t *h, const void *buf, uint32_t len, struct ffw_sockaddr *dest, int destlen);
 ...
 * @retval FFW_R_FAILED: 失败 
 */
int ffw_net_recvfrom1(ffw_net_handler_t *h, void *buf, uint32_t len, uint32_t *read_len, struct ffw_sockaddr *from, int *fromlen);
...
* @param h 句柄
* @param netif 监听网卡
* @param family FFW_AF_INET or FFW_AF_INET6
* @param port 监听端口
* @param proto FFW_IPPROTO_TCP or FFW_IPPROTO_TCP
* @return int 
* @retval FFW_R_SUCCESS: 成功
* @retval FFW_R_FAILED: 失败
 */
 ...
 * @retval FFW_R_FAILED: 失败 
 */
int ffw_net_accept(ffw_net_handler_t *h, struct ffw_sockaddr *addr, int *addrlen, ffw_net_handler_t *h_out);
 ...
 * @retval FFW_R_FAILED: 失败
 */
int ffw_net_server_transport_init(ffw_transport_t *t, ffw_handle_t netif, int family, uint16_t port, int proto);
