...
* @param arg 回调上下文
 */
typedef void (*on_data_ind_t)(ffw_httpc_t *c, void *arg);
...
struct ffw_httpc_s
{
    ...
    /// FFW_AF_INET FOR ipv4 AND FFW_AF_INET6 FOR IPV6
    int family;
 ...
 * @retval FFW_R_FAILED: 失败 
 */
int ffw_httpc_pause(ffw_httpc_t *c);
 ...
 * @retval FFW_R_FAILED: 失败 
 */
int ffw_httpc_resume(ffw_httpc_t *c);
...
* @retval FFW_R_SUCCESS: 成功
* @retval FFW_R_FAILED: 失败 
* @retval FFW_R_SOCKET_SEND_AGAIN 发送缓冲区满了，等待 on_send 函数调用之后继续发送
 */
int ffw_httpc_send_data1(ffw_httpc_t *c, uint8_t *data, uint32_t len, bool end);
...
* @param data 发送的数据
* @param len 发送数据的长度
* @param[out] sendlen 已发送数据的长度
* @return int 
* @retval FFW_R_SUCCESS: 成功
* @retval FFW_R_FAILED: 失败 
 */
int ffw_httpc_send_data2(ffw_httpc_t *c, uint8_t *data, uint32_t len, uint32_t *sendlen);
...
* @retval FFW_R_FAILED: 失败 
 */
int ffw_httpc_compress_enable(ffw_httpc_t *c);
...
* @return int 
 */
int ffw_httpc_read_data_async(ffw_httpc_t *c, bool with_header);

#ifdef CONFIG_SSL_GM_ENABLE
int ffw_httpc_request_recv_timeout_sync(ffw_httpc_t *c, ffw_http_method_t method, const char *url, bool redir, int timeout, void *arg);
#endif
