...
#ifdef FIBOCOM_BASE
    uint32_t    (*readId)               (lcdDrvFunc_t *lcd);
    void        (*sleep)                (lcdDrvFunc_t *lcd,uint8_t mode);
    int32_t     (*doSetPixel)           (lcdDrvFunc_t *lcd ,uint16_t x, uint16_t y, uint8_t val);
    void        (*dotSendBuff)          (lcdDrvFunc_t *lcd ,uint8_t column, uint8_t row, void *buff, uint32_t size);
    void        (*dotSetDisaplayParam)  (lcdDrvFunc_t *lcd ,uint8_t contrastRatio_coarse, uint8_t contrastRatio_fine, uint8_t bias);
    void        (*dotSetFlushDir)       (lcdDrvFunc_t *lcd ,uint8_t direction);
#endif
}lcdDrvFunc_t;
