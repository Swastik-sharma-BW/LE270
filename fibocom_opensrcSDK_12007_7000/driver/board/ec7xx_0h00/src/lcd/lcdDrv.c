...
lcdDrvFunc_t* lcdDevHandle;
lcdIoCtrl_t lcdIoCtrlParam;

extern lcdDrvFunc_t st7789Drv;
lcdDrvFunc_t *lcdDrv_handle = &st7789Drv;
/*
extern lcdDrvFunc_t st7567Drv;
lcdDrvFunc_t *lcdDrv_handle = &st7567Drv;
*/

lcdDrvFunc_t* lcdOpen(void* uspCb, void* dmaCb)
{
    int32_t ret = 0;
    ret = lcdDrv_handle->init(lcdDrv_handle, uspCb, dmaCb, lcdDrv_handle->freq, lcdDrv_handle->bpp);
    if (ret)
    {
        return NULL;
    }
    lcdDevHandle = lcdDrv_handle;
    return lcdDrv_handle;
}
...
void lcdRegInit(uint32_t id)
{
    ...
    init = lcdDrv_handle->initRegTbl;

    for (int j = 0; j < lcdDrv_handle->initRegTblLen; j++)
    {
        if (init->cmd == 0xff)
        {
...
void lcdSleep(lcdDrvFunc_t *lcd, uint8_t mode)
{
    if (lcd == NULL)
    {
        return;
    }
    return lcd->sleep(lcd, mode);
}

void lcdSetFlushDir(lcdDrvFunc_t *lcd, uint8_t direction)
{
    if (lcd == NULL)
    {
        return;
    }
    return lcd->dotSetFlushDir(lcd, direction);
}
#else
