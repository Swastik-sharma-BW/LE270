...
uint32_t TIMER_isRunning(uint32_t instance);
#endif
/**
...
#ifdef FIBOCOM_BASE
PLAT_FM_RAMCODE uint32_t TIMER_getCount1(uint32_t instance);
int32_t TIMER_setupPwm1(uint32_t instance, const TimerPwmConfig_t *config);
PLAT_FM_RAMCODE void TIMER_updatePwmDutyCycle1(uint32_t instance, uint32_t dutyCyclePercent);
PLAT_FM_RAMCODE uint32_t TIMER_getInterruptFlags1(uint32_t instance);
PLAT_FM_RAMCODE void TIMER_clearInterruptFlags1(uint32_t instance, uint32_t mask);
#endif
