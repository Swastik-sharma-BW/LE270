...
#ifdef FIBOCOM_BASE
/*
 * Fibocom NV save APNV_RESV_PART:AP_NV_RESV_OFFSET(NVRAM_PHYSICAL_BASE+0x1000+0x20(NvHeader_t)),
 *                                 this is resv for fibocom by liuben
 * FIBO_NV_DATA_LEN : ETH_MAC(16Byte) + secboot_flag(8Byte) + imei2(32Byte) + sn2(32Byte)
 */
#define FIBO_NV_DATA_LEN                (16 + 8 + 32 + 32)

#define FIBO_NV_DATA_ETH_MAC_LEN        16
#define FIBO_NA_DATA_SECBOOT_FLAG_LEN   8
#define FIBO_NV_DATA_ETH_MAC_OFFSET     0
#define FIBO_NV_DATA_SECBOOT_FLAG_OFFSET  FIBO_NV_DATA_ETH_MAC_OFFSET + FIBO_NV_DATA_ETH_MAC_LEN

#define FIBO_NV_DATA_IMEI2_LEN          32
#define FIBO_NV_DATA_IMEI2_OFFSET       FIBO_NV_DATA_SECBOOT_FLAG_OFFSET + FIBO_NA_DATA_SECBOOT_FLAG_LEN

#define FIBO_NV_DATA_SN2_LEN            32
#define FIBO_NV_DATA_SN2_OFFSET         FIBO_NV_DATA_IMEI2_OFFSET + FIBO_NV_DATA_IMEI2_LEN
#endif
...
//2024-09-02, mahongmei, MTC0832-448, Begin. fix wifiscan bug.
#ifdef FIBOCOM_BASE
#define WIFISCAN_0_TIME_VAL_MIN             1000  //modified by fibocom,MTC0709-889
#ifdef CONFIG_FFW_MEITUAN_CONFIG
#define WIFISCAN_0_TIME_VAL_MAX             45*1000 
#else
#define WIFISCAN_0_TIME_VAL_MAX             30*1000  //modified by fibocom,MTC0709-889
#endif
#else
...
#ifdef FIBOCOM_BASE
#define WIFISCAN_2_MAXBSSIDNUM_VAL_MIN      1 //modified by fibocom,MTC0709-889
#define WIFISCAN_2_MAXBSSIDNUM_VAL_MAX      30 //modified by fibocom,MTC0709-889
#else
#define WIFISCAN_2_MAXBSSIDNUM_VAL_MIN      4
#define WIFISCAN_2_MAXBSSIDNUM_VAL_MAX      40
#endif
//2024-09-02, mahongmei, MTC0832-448, End. 
...
CmsRetId appSetCSIMSync(UINT16 cmdApduStrLen,
                                ...
                                UINT8 *rspApduStr);
/**
  \fn          appSetCSIMSyncOpt
  \brief       Send cmi request to transmit APDU command to the SIM and get APDU response from the SIM
  \param[in]   UINT16 cmdApduStrLen, the length of command APDU string
  \param[in]   UINT8 *cmdApduStr, the pointor to the command APDU, hex string
  \param[out]  UINT16 *rspApduStrLen, the pointor to the length of response APDU string, shall less than rspApduBufSize
  \param[out]  UINT8 **rspApduStr, the pointor to the pointer of the response APDU, hex string. Waining!!! memory allocated here and free by caller if not null.
  \returns     CmsRetId
  \NOTE:       refer: AT+CSIM
  \            shall set SIM sleep not allowed (power on SIM) by appSetEcSIMSleepSync first if enable SIM power save,
  \            then set SIM sleep allowed (power off SIM) to save power after finish if required.
  \            e.g.
  \            CHAR cmdApduStr[15] = "00A40004023F00";
  \            UINT16 cmdApduLen = 0;
  \            UINT16 rspApduStrLen = 0;
  \            UINT8 *rspApduStr = PNULL;
  \            cmdApduLen = strlen(cmdApduStr);
  \            appSetEcSIMSleepSync(FALSE);
  \            appSetCSIMSyncOpt(cmdApduLen, (UINT8 *)cmdApduStr, &rspApduStrLen, &rspApduStr);
  \            if (rspApduStr != PNULL)
  \            {
  \                 OsaFreeMemory(&rspApduStr);
  \            }
  \            appSetEcSIMSleepSync(TRUE);
*/
CmsRetId appSetCSIMSyncOpt(UINT16 cmdApduStrLen,
                                 UINT8 *cmdApduStr,
                                 UINT16 *rspApduStrLen,
                                 UINT8 **rspApduStr);
...
*/
#if defined(FIBOCOM_BASE)
CmsRetId appSendSmsSync(UINT8 smsFormat, CHAR *da, UINT8 toda,
...
#if defined(FIBOCOM_BASE)
CmsRetId fibo_appSmsReadTextSmsFromStorageSync(UINT8 smsMemType, UINT8 memIndex, PsilSmsTimeStampInfo *SmsTimeStamp,SmsInfo *pSmsInfo,PsilSmsStoreItemInfo *pReadSmsInfo,UINT8 *pUserDataHead);
#endif

#endif
#if defined(FIBOCOM_BASE)
CmsRetId appGetEpsBeaerParamSync(const UINT8  cid,GetPsBearerParams *pGetEpsBrParams);
CmsRetId appSetEpsBeaerParamSync(SetPsBearerParams *pSetEpsParams);
CmsRetId appGetRemainingPINRetriesSync(UINT8 sleCode,RemianPinRetriesParams *pRemianPinRetriesParams);

CmsRetId appGetImei2NumSync(CHAR *imei);
BOOL appSetImei2NumSync(CHAR* imei);
CmsRetId appGetSimMccMncSync(CHAR *mcc,CHAR *mnc);
CmsRetId appGetAllBandCellInfoSync(BasicCellListInfo *pBcListInfo);
CmsRetId appGetSecbootFlagSync(CHAR* secboot_flag);
BOOL appSetSecbootFlagSync(CHAR* secboot_flag);
BOOL appGetSN2NumSync(CHAR* sn);
BOOL appSetSN2NumSync(CHAR* sn, UINT8 len);
CmsRetId fibo_appGetECBCInfoSync(BasicCellListInfo *pBcListInfo,CmiDevGetBasicCellListInfoReq *cmiReq);
#endif
#ifdef CONFIG_FFW_FEATURE_IEEE_MAC
CmsRetId appGetEthMacNumSync(CHAR* eth_mac);
BOOL appSeEthMacNumSync(CHAR* eth_mac);
#endif
/**
...
/**
  \fn          BOOL appSetSoftRstAuthRej
  \brief       set the soft reset interval for Authentication Reject
  \param[in]   bEnable
  \param[in]   rstTimesStp1, rstTimesStp2:
  \              soft reset times for step1,2
  \param[in]   delayMinSec, delayMaxSec
  \              wait for random seconds from delayMinSec to delayMaxSec before soft reset
  \param[in]   minSecBeforeStp2, maxSecBeforeStp2:
  \              wait for random seconds from minSecBeforeStp2 to maxSecBeforeStp2 for the
  \              first soft reset of step2
  \returns     CmsRetId
*/
CmsRetId appSetSoftRstAuthRej(BOOL bEnable,
                              UINT8 rstTimesStp1, UINT8 rstTimesStp2,
                              UINT8 delayMinSec, UINT8 delayMaxSec,
                              UINT8 minSecBeforeStp2, UINT8 maxSecBeforeStp2);
