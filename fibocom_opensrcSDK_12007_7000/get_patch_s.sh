...
init()
{
	...
	vendor/mediatek/proprietary/scripts/sign-image/obj/SignLib.a
	vendor/mediatek/proprietary/scripts/sign-image/SignTool_MTK
	Mota/*
	get_patch_s.sh
	release/
	"
...
get_patch()
{
	...
	if [ "$git_version" == "git version 1.9.1" ]; then
		changed_fileinfos=$(git status | awk '{if ($1 =="'$modified_tag'") {print "'$mf_tag'"$2} 
									   else if ($1 =="'$deleted_tag'") {print "'$df_tag'"$2}
									   else if ($1 !="'$modified_tag'" && $1 !="'$deleted_tag'") {print "'$aa_tag'"$1}
									   }')
	else
