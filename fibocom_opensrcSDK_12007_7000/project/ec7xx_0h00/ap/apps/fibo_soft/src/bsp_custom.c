...
__USED static uint8_t rsvUsrRam[4];    //[115*1024];
#else
__USED static uint8_t rsvUsrRam[4];    //[120*1024];
#endif
#else
#ifdef FEATURE_EXCEPTION_FLASH_DUMP_ENABLE
__USED static uint8_t rsvUsrRam[4];    //[295*1024];
#else
__USED static uint8_t rsvUsrRam[4];    //[300*1024];
#endif
#endif
#else
#ifdef FEATURE_EXCEPTION_FLASH_DUMP_ENABLE
__USED static uint8_t rsvUsrRam[4];    //[195*1024];
#else
__USED static uint8_t rsvUsrRam[4];    //[200*1024];
#endif
#endif
#else//718
#ifdef OPEN_CPU_MODE
...
#ifdef FEATURE_EXCEPTION_FLASH_DUMP_ENABLE
__USED static uint8_t rsvUsrRam[4];    //[71*1024];     // 71KB ram reserved for customer
#else
__USED static uint8_t rsvUsrRam[4];    //[79*1024];     // 79KB ram reserved for customer
#endif

#else

#ifdef FEATURE_EXCEPTION_FLASH_DUMP_ENABLE
__USED static uint8_t rsvUsrRam[4];    //[41*1024];     // 41KB ram reserved for customer
#else
__USED static uint8_t rsvUsrRam[4];    //[49*1024];     // 49KB ram reserved for customer
#endif
...
#ifdef FEATURE_EXCEPTION_FLASH_DUMP_ENABLE
__USED static uint8_t rsvUsrRam[4];    //[337*1024];     // 337KB ram reserved for customer
#else
__USED static uint8_t rsvUsrRam[4];    //[345*1024];    // 345KB ram reserved for customer
#endif

#else

#ifdef FEATURE_EXCEPTION_FLASH_DUMP_ENABLE
__USED static uint8_t rsvUsrRam[4];    //[415*1024];     // 415KB ram reserved for customer
#else
__USED static uint8_t rsvUsrRam[4];    //[420*1024];    // 420KB ram reserved for customer
#endif
#endif
#endif
#else//not open cpu
...
#ifdef FEATURE_EXCEPTION_FLASH_DUMP_ENABLE
__USED static uint8_t rsvUsrRam[4];    //[152*1024];     // ccid feature need more SRAM
#else
__USED static uint8_t rsvUsrRam[4];    //[160*1024];    // ccid feature need more SRAM
#endif

#else
#ifdef FEATURE_EXCEPTION_FLASH_DUMP_ENABLE
__USED static uint8_t rsvUsrRam[4];    //[162*1024];    // 162KB ram reserved for customer
#else
__USED static uint8_t rsvUsrRam[4];    //[170*1024];    // 174KB ram reserved for customer
#endif
...
#ifdef CONFIG_FFW_MIFI_ENABLE
UINT8 gChargeStatus = CHARGE_STATUS_DISCONNECT;
#endif
...
void WakeupPadInit(void)
{
    ...
    //all others disable and set as floating
...
#ifdef FIBOCOM_BASE
#ifdef CONFIG_FFW_KEY_ENABLE
    if(ffw_key_get_has_registered()) ffw_key_pwrkey_cb(status);
#endif
    if((status == PWRKEY_LONGPRESS)&&(gIspwroff))
#else
...
#ifdef FIBOCOM_BASE
	usblpw_innophy_enh_drv_strenth();  //add By Lucien
#endif
#endif
...
#if defined FIBOCOM_BASE && defined CHIP_EC716
...
void BSP_CustomInit(void)
{
...
#ifdef FIBOCOM_BASE
    extern void *ffw_psram_malloc_plat(uint32_t size);
    extern    void ffw_psram_free_plat(void *p);
    char *ptr = ffw_psram_malloc_plat(4);
    ECPLAT_PRINTF(UNILOG_PLA_STRING, build_info_psram, P_SIG, "init psram point: %p", ptr);
    if (NULL != ptr)
    {
        ffw_psram_free_plat(ptr);
    }
#endif
#endif
#else
...
#ifndef CONFIG_UART_AT_DEFAULT_DISABLED
    SetAtUart(PORT_USART_1);
#endif
#if (RTE_TWO_UART_AT == 1)
#ifdef CONFIG_FFW_MIFI_ENABLE
    SetAtUart(PORT_USART_0);
#else
...
#if defined FIBOCOM_BASE && defined CONFIG_FFW_LPM_SUPPORT
    slpManSetPmuSleepMode(true, SLP_IDLE_STATE, false); //set Sleep level,Platform Features!
    slpManAONIOPowerOn();
#endif

#ifdef CONFIG_FFW_FEATURE_DEFAULT_IO_3V3
...
#if defined FIBOCOM_BASE && defined CHIP_EC716
    //EC716 sim2 pin init out low
    Usim1GpioConfig(false);
#endif

#ifdef CONFIG_FFW_VDD_EXT_POWER_BY_AGPIO
    //AGPIO3 as VDD_EXT POWER
    ffw_gpiomode_set(0, 0);
    ffw_gpio_setDir(0, FFW_DRV_GPIO_OUTPUT);
    ffw_gpio_setLevel(0, FFW_HAL_GPIO_DATA_HIGH);
#endif
    powerKeyFuncInit();
#ifdef FIBOCOM_BASE
    XIC_SuppressOvfIRQ(PXIC0_TIMER0_IRQn);
    XIC_SuppressOvfIRQ(PXIC0_TIMER1_IRQn);
    XIC_SuppressOvfIRQ(PXIC0_TIMER2_IRQn);
    XIC_SuppressOvfIRQ(PXIC0_TIMER3_IRQn);
    XIC_SuppressOvfIRQ(PXIC0_TIMER4_IRQn);
#endif

#ifdef FIBOCOM_BASE //add by liuben
    ffw_pal_device_update_mac();
#endif
    ...
    ResetReasonInit();
#if defined(FIBOCOM_BASE)&&(defined TYPE_EC718P)
...
#ifdef CONFIG_FFW_MIFI_ENABLE
    SFDT_slaveSrdyInit();
    chargeDetectInit(chargeStatusCallbackFunc,TRUE,1000);
#endif
#if defined(CONFIG_FFW_CUST_ETHER_ENABLE)
    CECS_MasterGpioInit();
#endif
...
pfn_PadWakeupHook p_LpmWakeupHook = NULL;
void RegLpmWakeupIntrHook(pfn_PadWakeupHook pfunc)
{
    if(p_LpmWakeupHook == NULL)
        p_LpmWakeupHook = pfunc;
}
#endif

#ifdef FIBOCOM_BASE
pfn_PadWakeupHook p_PinWkeupHook = NULL;
void RegPinWkeupIntrHook(pfn_PadWakeupHook pfunc)
{
    if(p_PinWkeupHook == NULL)
        p_PinWkeupHook = pfunc;
}
#endif
...
#ifdef CONFIG_FFW_LPM_SUPPORT
    if (E_DRIVER_LE230_PROJECT == fibo_dirver_get_project()) //LE230 WAKEUP_IN
    {
        if (p_LpmWakeupHook)
            (*p_LpmWakeupHook)(PadWakeup0_IRQn);
    }
#endif

#ifdef FIBOCOM_BASE
    //release WAKEUP0 for opencpu
    if(p_PinWkeupHook)
        (*p_PinWkeupHook)(PadWakeup0_IRQn);
#endif

#ifdef FIBOCOM_BASE
#if defined  TYPE_EC716S || defined TYPE_EC716E //716S or 716E VBUS
    if (p_funcPadWakeupHook)
    {
        (*p_funcPadWakeupHook)(0);
    }
#endif
#endif
}
...
#ifdef CONFIG_SUPPORT_USIM1_DETECT
pfn_PadWakeupHook p_simdetWakeupHook = NULL;
int RegSimDetWakeupIntrHook(pfn_PadWakeupHook pfunc)
{
    if (p_simdetWakeupHook)
    {
        return -1;
    }
    p_simdetWakeupHook = pfunc;
    return 0;
}
#endif
...
void Pad1_WakeupIntHandler(void)
{
    ...
    uniLogFlushOut();
#ifdef FIBOCOM_BASE
#if defined  TYPE_EC716S || defined TYPE_EC716E
        //release EC716 SIM_DET for opencpu
        if (p_PinWkeupHook)
            (*p_PinWkeupHook)(PadWakeup1_IRQn);
#endif
#endif

#if defined CONFIG_SUPPORT_USIM1_DETECT && (defined  TYPE_EC716S || defined TYPE_EC716E) //716S or 716E SIM_DET
    if (p_simdetWakeupHook)
    {
        (*p_simdetWakeupHook)(PadWakeup1_IRQn);
    }
#else //718S or 718P VBUS
    if (p_funcPadWakeupHook)
    {
...
#ifdef FIBOCOM_BASE
#if !defined  TYPE_EC716S && !defined TYPE_EC716E
        //release EC718 SIM_DET for opencpu
        if (p_PinWkeupHook)
            (*p_PinWkeupHook)(PadWakeup2_IRQn);
#endif
#endif

#ifdef CONFIG_SUPPORT_USIM1_DETECT
#if !defined  TYPE_EC716S && !defined TYPE_EC716E //718s and 718p SIM_DET
    if (p_simdetWakeupHook)
    {
        (*p_simdetWakeupHook)(PadWakeup2_IRQn);
    }
#endif
#else
#ifdef SIM_HOT_SWAP_FEATURE
...
#ifdef CONFIG_FFW_LPM_SUPPORT
    if (E_DRIVER_LE371_PROJECT == fibo_dirver_get_project()) //LE371 DTR
    {
        if (p_LpmWakeupHook)
            (*p_LpmWakeupHook)(PadWakeup3_IRQn);
    }
#endif

#ifdef FIBOCOM_BASE
    if(p_PinWkeupHook)
        (*p_PinWkeupHook)(PadWakeup3_IRQn);
#endif
...
pfn_PadWakeupHook p_wdisableWakeupHook = NULL;
int RegWdisableWakeupIntrHook(pfn_PadWakeupHook pfunc)
{
    if (p_wdisableWakeupHook)
    {
        return -1;
    }
    p_wdisableWakeupHook = pfunc;
    return 0;
}
#endif
...
void Pad4_WakeupIntHandler(void)
{
    ...
    if (p_wdisableWakeupHook)
    {
        (*p_wdisableWakeupHook)(PadWakeup4_IRQn);
    }
#endif

#ifdef FIBOCOM_BASE
    if(p_PinWkeupHook)
        (*p_PinWkeupHook)(PadWakeup4_IRQn);
#endif
...
#ifdef CONFIG_FFW_LPM_SUPPORT
    if (p_LpmWakeupHook)
        (*p_LpmWakeupHook)(PadWakeup5_IRQn);
#else
    if (p_funcPadWakeupHook)
    {
...
#ifdef FIBOCOM_BASE
    if(p_PinWkeupHook)
        (*p_PinWkeupHook)(PadWakeup5_IRQn);
#endif
...
void Usim1ClkLatchByAONIO(BOOL bLatchOnHigh)
{
    ...
    */
    ...
    /*
    * configure EC718:AONIO-28  EC716:AONIO:GPIO26
    */
    PAD_getDefaultConfig(&padConfig);
    padConfig.mux = PAD_MUX_ALT0;
    PAD_setPinConfig(USIM1_LATCH_IO_PAD_INDEX, &padConfig);
    config.pinDirection = GPIO_DIRECTION_OUTPUT;
    if (bLatchOnHigh == TRUE)
    {
        config.misc.initOutput = 1;
    ...
    GPIO_pinConfig(USIM1_LATCH_IO_INSTANCE, USIM1_LATCH_IO_PIN, &config);
...
extern void XpIdleTimeStatisPrint(uint32_t printBlock);
void apPrintIdlePercent(void)
{
    AP_PLAT_COMMON_BSS static osTimerId_t ap20msTimer;
    uint32_t idlePrintMode = 0;
    osStatus_t ret = 0;

    idlePrintMode = BSP_GetPlatConfigItemValue(PLAT_CONFIG_ITEM_IDLEPERCENT_PRINT_MODE);

    if(idlePrintMode)
    {
        if(ap20msTimer == NULL)
        {
            ap20msTimer = osTimerNew((osTimerFunc_t)XpIdleTimeStatisPrint, osTimerPeriodic, (void *)(uint32_t)idlePrintMode, NULL);
        }
#ifdef TYPE_EC718M
        ret = osTimerStart(ap20msTimer, 20*(idlePrintMode&0x0f));
#else
        ret = osTimerStart(ap20msTimer, 20*idlePrintMode);
#endif
        if(ret != 0)
        {
            ECPLAT_PRINTF(UNILOG_PLA_DRIVER, apPrintIdlePercent_1, P_WARNING, "Start Idle Percent Timer Fail!");
        }
    }
}
