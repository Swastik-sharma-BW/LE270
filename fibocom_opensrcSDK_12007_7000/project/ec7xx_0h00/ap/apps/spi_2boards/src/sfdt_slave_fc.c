...
#define SLAVE_TRANSFER_TRIGGER_FLAG          (SLAVE_TRANSFER_TX_FLAG | SLAVE_TRANSFER_RX_FLAG | SLAVE_TRANSFER_FLOW_CONTROL_FLAG)
...
CHECK_TX:
            if(gMasterFlowControlState == SFDT_FLOW_CONTROL_XON)
            {
            ...
            if((errCode == CCIO_EFOUND) && (eventFlags == 0))
            {
                ...
                EC_ASSERT(eventFlags < osFlagsError, eventFlags, 0 , 0);
            ...
            else
            {
                ...
                header = (errCode == CCIO_EOK) ? (SFDTFrameHeader_t *)txBufDesc.buf : &gSlaveTransferHeader;
                memset(header, 0, SFDT_FRAME_HEADER_SIZE);

                header->field.payLoadLen = (errCode == CCIO_EOK) ? txBufDesc.len - SFDT_FRAME_HEADER_SIZE : 0;
                header->field.flowControl = gSlaveFlowControlState; // Need atomic op?
                header->field.seqNo = txSeqNo++;
                header->field.checkSum = SFDT_csum(header);
                txPtr = (uint8_t *)header;
                txNum = (errCode == CCIO_EOK) ? txBufDesc.len : SFDT_FRAME_HEADER_SIZE;
                pendingTxCnt = txNum;
...
#ifdef SFDT_SLAVE_LOG_ENABLE
        ECPLAT_PRINTF(UNILOG_PLA_APP, SFDT_slaveTransferTaskEntry_3, P_WARNING, "slave is ready to transfer, txNum: %d", pendingTxCnt);
#endif
...
HANDSHAKE:
        ...
        {
            EC_ASSERT(eventFlags == SLAVE_TRANSFER_DONE_FLAG, eventFlags, 0 , 0);
        }
        ...
        if(SFDT_csum(header) == header->field.checkSum)
        {
            ...
            if((rxPayLoadLen + SFDT_FRAME_HEADER_SIZE) <= txCnt)
            {
                ...
                if(gMasterFlowControlState != header->field.flowControl)
                {
                    gMasterFlowControlState = header->field.flowControl;
                    spiDevNotifyEvent(SPI_DEV_EVENT_FC, &gMasterFlowControlState);
                }

                //No need to get rx buf in case of no data payload
                if(rxPayLoadLen != 0)
                {
                    ...
                    if(rxNewBuffer)
                    {
                        spiDevInput(SLAVE_DEVICE_INDEX, rxPayLoadLen, 0);
...
{
    slpManAONIOPowerOn();
	slpManNormalIoPowerCtrl(IOVOLT_3_30V);
