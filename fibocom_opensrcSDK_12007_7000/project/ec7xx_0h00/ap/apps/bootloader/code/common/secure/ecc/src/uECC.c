...
// to be fixed for uecc random gen
uint64_t random_num[4]={0x1234567887654321,0x1234567887654321,0x1234567887654321,0x1234567887654321};
...
extern uint64_t random_num[4];

static int getRandomNumber(uint8_t *dest, unsigned size)
{
	memcpy(dest,(uint8_t *)random_num,32);
	return 1;
...
#else
static uECC_RNG_Function g_rng_function = &getRandomNumber;
#endif
...
int uECC_shared_secret(const uint8_t *public_key,
    ...
    memmove(_private, private_key, num_bytes);
    memmove(_public, public_key, num_bytes*2);
#endif
...
#elif defined (__GNUC__)
    memmove(public_key, compressed+1, curve->num_bytes);
#endif
...
#elif defined (__GNUC__)
    memmove(tmp, private_key, BITS_TO_BYTES(curve->num_n_bits));
#endif
...
#elif defined (__GNUC__)
    memmove(r, signature, curve->num_bytes);
    memmove(s, signature + curve->num_bytes, curve->num_bytes);
#endif
