...
int app_mqtt_start_demo()
{
    ...
    {
        fibo_mutex_unlock(g_sig_mqtt_lock);
        int ret = app_mqtt_open(id, username, password, client_id, (INT8 *)serv_addr, port, clear_session, keepalive, UseTls);
        if (ret != 0)
        {
            APP_LOG_INFO("mqtt login fail");
            fibo_taskSleep(1000);
        ...
        ret = app_mqtt_sub(id, (INT8 *)topic, qos);
        if (ret != 0)
        {
            APP_LOG_INFO("mqtt sub fail");
        }

        ret = app_mqtt_sub(id, (INT8 *)"unsubtopic", qos);
        if (ret != 0)
        {
            APP_LOG_INFO("mqtt sub fail");
        }

        ret = app_mqtt_unsub(id, (INT8 *)"unsubtopic");
        if (ret != 0)
        {
            APP_LOG_INFO("mqtt unsub fail");
        }

        ret = app_mqtt_pub(id, topic, qos, retain, (INT8 *)pubmessg, sizeof(pubmessg) - 1);
        if (ret != 0)
        {
...
int app_mqtt_demo()
{
    while (true)
    {
        fibo_thread_create(mqtt_task, (INT8 *)"mqtttask", 10 * 1024, NULL, 24);
