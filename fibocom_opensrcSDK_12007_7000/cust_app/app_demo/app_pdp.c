...
int app_pdp_active(int simid, int cid, app_pdp_iptype_t iptype, char *apn, char *apnuser, char *apnpasswd, app_pdp_authproto_t auth_proto)
{
    ...
    if (cid == 1) /// 如果cid为1，则使用默认承载
    {
        if (apn) /// 改变默认承载配置
        {
            ...
            fibo_mutex_unlock(g_sig_reg_lock);
            fibo_cfun_set(0, simid);
            fibo_cfun_set(1, simid);
            fibo_mutex_lock(g_sig_reg_lock);
        }
    }
    else
    {
        ...
        if (apnuser && apnpasswd)
        {
            ...
            pdp_profile.apnUserSize = strlen((char *)pdp_profile.apnUser);
            pdp_profile.apnPwdSize = strlen((char *)pdp_profile.apnPwd);
            pdp_profile.nAuthProt = auth_proto;
        }
    }

    reg_info.nStatus = 0;
    fibo_reg_info_get(&reg_info, simid);
    g_network_status = reg_info.nStatus;
...
int app_pdp_check_pdp_release(int cid)
{
    ...
    fibo_mutex_lock(g_sig_reg_lock);
    APP_WAIT_EXPECT_STATUS_TIMEOUT(g_sig_reg_lock, g_app_pdp_info[cid].sem, (g_app_pdp_info[cid].status != APP_PDP_STATUS_DEACTIVED), g_app_pdp_info[cid].status, 10 * 1000);
    fibo_mutex_unlock(g_sig_reg_lock);
...
int app_pdp_demo()
{
    ...
    while (true)
    {
        ...
        if (ret != 0)
        {
            fibo_printf("pdp active fail cnt=%d", cnt);
            fibo_taskSleep(1000);
            continue;
        }
        //app_pdp_check_pdp_release(cid);
        return 0;
    }
}
