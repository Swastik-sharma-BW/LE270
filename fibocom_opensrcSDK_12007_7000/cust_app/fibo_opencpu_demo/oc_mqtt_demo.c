...
static void mqtt_thread_entry(void *param)
{
    ...
    fibo_mqtt_set((int8_t*)"test", (int8_t*)"123");
    ...
    if (connect_res_result)
    {
        ...
        ret = fibo_mqtt_sub_v2((int8_t*)topic, qos, &pacid);
        if (ret < 0)
        {
            fibo_textTrace("mqttapi sub failed");
        }
        ret = fibo_mqtt_pub_v2((int8_t*)topic, qos, retain, (int8_t*)message, strlen(message), &pacid);
        if (ret < 0)
        {
            fibo_textTrace("mqttapi sub failed");
        }
