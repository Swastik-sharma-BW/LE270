...
static void fibo_get_nmea_data_and_gnss_status(void)
{
    ...
    char *str_tmp = NULL;
    int i = 0;
    local_time_t time_value;
    local_time_t time_value_1;
    bool location_ok = false;
    char nmea_rmc[20][20] = {0};

    fibo_local_time_get(&time_value);
    ...
    for (int n = 0; n < 10000; n++)
    {
        ...
        if (location_ok)
        {
            fibo_local_time_get(&time_value_1);
...
int appimg_enter(void *param)
{
    ...
    fibo_thread_create(gnss_thread_entry, (INT8 *)"mythread", 8*1024, NULL, OSI_PRIORITY_NORMAL);
    return 0;
}
